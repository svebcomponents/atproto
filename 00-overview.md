...
**Foundation**: scaffolded from `svebcomponents/template` (pnpm + turbo, `apps/` + `components/` + `configs/`). The template's SvelteKit app becomes both the component showcase _and_ the hosted OAuth service. A new `packages/` group holds framework-agnostic logic (thread fetching/normalization, AT URI handling, service client).
...
1. **Embedded auth across origins** ([03-oauth-service.md](./03-oauth-service.md)): the component runs on `someblog.com`, while the hosted service runs on `atproto.svebcomponents.dev`. Third-party cookies are blocked by all major browsers in 2026, so hosted mode uses a popup OAuth flow, COOP-safe claim handoff, and short-lived origin-bound bridge JWT. A same-origin self-host switches the same client APIs to an HttpOnly cookie. Real OAuth tokens never leave the service.
2. **SSR without double-render jank** ([01-architecture.md](./01-architecture.md)): Svelte-compiled custom elements re-render from scratch when they upgrade on the client — declaratively-rendered shadow DOM is replaced, and a naive component would refetch, causing a flash. A preloaded `threadData` channel makes the client render instant from embedded data; the hosted SSE connection then provides an event-driven freshness boundary rather than a time-based stale interval.
