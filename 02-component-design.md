...
- Attribute→prop conversion is generated by the build from the component's `$props()` and `<svelte:options customElement>` declaration; keep prop types simple (string/number/boolean) at the attribute boundary.
