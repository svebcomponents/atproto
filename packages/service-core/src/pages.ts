...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .after { margin-top: 1rem; font-size: .85rem; text-align: center; }
...
${
  brand
    ? `<${brand.url ? `a class="brand" href="${escapeHtml(brand.url)}"` : `div class="brand"`}>${BRAND_MARK}<span>${escapeHtml(brand.name)}</span></${brand.url ? "a" : "div"}>`
    : ""
}
${body}
</div>
